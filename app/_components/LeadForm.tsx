...
export function LeadForm({ intent }: { intent: "hospital_demo" | "investor_access" }) {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    if (!response.ok) { const data = await response.json().catch(() => ({})); setError(data.error || "We could not send your request. Please try again or email hello@horalix.com."); setState("error"); return; }
  ...
  if (!opened) return <button className="button button-dark" data-track={intent === "hospital_demo" ? "demo_start" : "investor_start"} onClick={() => setOpened(true)}>Start secure request <span>→</span></button>;
  ...
  return (
    <form className="lead-form" onSubmit={submit}>
      ...
      <label className="consent"><input required type="checkbox" name="consent" value="yes" /> <span>I agree that Horalix may use these details to respond to my request. See the <a href="/privacy">privacy notice</a>.</span></label>
      <p className="form-error" id={`${intent}-error`} role="alert" aria-live="polite">{error}</p>
