...
const people = [
  ...
  { id: "amr-husain", name: "Amr Husain", role: "CFO & Co-Founder", focus: "Finance, operations, and company development", image: "/team/amr-husain.png", initials: "AH" },
  { id: "affan-kapidzic", name: "Affan Kapidzic", role: "CTO", focus: "Platform architecture and software engineering", image: "/team/affan-kapidzic.png", initials: "AK" },
  { id: "neuman-alkhalil", name: "Neuman Alkhalil", role: "CSO", focus: "Machine learning and model evaluation", image: "/team/neuman-alkhalil.png", initials: "NA" },
];
...
export default function About() {
  return <>
    ...
    <section className="section section-dark" id="team"><div className="shell"><div className="section-heading"><div><p className="eyebrow">The team</p><h2>Four disciplines. One clinical workflow.</h2></div><p className="lede">Horalix connects clinical workflow and product direction with company operations, platform engineering, and machine-learning evaluation around one echo-first focus.</p></div><div className="team-grid">{people.map((person, index) => <article id={person.id} className="team-card" key={person.name}>{person.image ? <Image src={person.image} alt={`${person.name}, ${person.role} at Horalix`} width={900} height={900} sizes="(max-width: 720px) 100vw, (max-width: 1024px) 50vw, 25vw" /> : <div className="team-monogram" aria-hidden="true"><span>{person.initials}</span><i /></div>}<div className="team-card-copy"><span className="team-index">0{index + 1}</span><h3>{person.name}</h3><b>{person.role}</b><p>{person.focus}</p></div></article>)}</div></div></section>
