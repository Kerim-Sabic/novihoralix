...
export default function About() {
  return <>
    ...
    <section className="section-tight shell" id="advisors"><div className="section-heading"><div><p className="eyebrow">Advisors</p><h2>Clinical and operating guidance from outside the company.</h2></div><p className="lede">Advisors challenge our clinical assumptions and company decisions. They are not employees, and advising does not imply endorsement of any specific product claim.</p></div><div className="advisor-grid">{advisors.map((advisor) => <article id={advisor.id} className="advisor-card" key={advisor.name}>{advisor.image ? <Image className="advisor-face" src={advisor.image} alt={`${advisor.name}, ${advisor.role} to Horalix`} width={480} height={480} sizes="(max-width: 720px) 50vw, 25vw" /> : <div className="advisor-face advisor-monogram" aria-hidden="true"><span>{advisor.initials}</span></div>}<h3>{advisor.name}</h3><b>{advisor.role}</b>{advisor.focus ? <p>{advisor.focus}</p> : null}{advisor.disclosure ? <small>{advisor.disclosure}</small> : null}</article>)}</div></section>
